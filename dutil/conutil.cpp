//-------------------------------------------------------------------------------------------------
// <copyright file="conutil.cpp" company="Microsoft">
...
extern "C" HRESULT DAPI ConsoleInitialize()
{
	...
	else
	{
		...
		{
			ExitOnFailure(hr = HRESULT_FROM_WIN32(er), "failed to get input console screen buffer info");
		}
	}
	...
	else   // no console
	{
		...
		{
			ExitOnFailure(hr = HRESULT_FROM_WIN32(er), "failed to get output console screen buffer info");
		}
	}
