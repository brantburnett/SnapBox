...
function Save-OpaqueBitmap([System.Drawing.Bitmap]$bitmap, [string]$path) {
    $opaqueBitmap = [System.Drawing.Bitmap]::new($bitmap.Width, $bitmap.Height, [System.Drawing.Imaging.PixelFormat]::Format24bppRgb)
    $graphics = [System.Drawing.Graphics]::FromImage($opaqueBitmap)
    $graphics.Clear([System.Drawing.Color]::White)
    $graphics.DrawImageUnscaled($bitmap, 0, 0)
    $graphics.Dispose()
    $opaqueBitmap.Save($path, [System.Drawing.Imaging.ImageFormat]::Bmp)
...
try {
    ...
    $panel.Dispose()
    $graphics.Dispose()
    Save-OpaqueBitmap $dialog (Join-Path $imagesDirectory 'WixUIDialog.bmp')
}
finally {
    $dialog.Dispose()
}
